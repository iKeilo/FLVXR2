...
func (h *Handler) licenseActivate(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		response.WriteJSON(w, response.ErrDefault("璇锋眰澶辫触"))
		return
	}

	now := time.Now().UnixMilli()
	if err := h.repo.UpsertConfig("license_key", "permanent-open-source", now); err != nil {
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	}
	...
	if err := h.repo.UpsertConfig("license_expiry", "never", now); err != nil {
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	}
