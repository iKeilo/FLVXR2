...
func (h *Handler) userCreate(w http.ResponseWriter, r *http.Request) {
	...
	if renewalAmount < 0 || balance < 0 {
		response.WriteJSON(w, response.ErrDefault("续费金额和余额不能小�?0"))
		return
	}
	roleID := 1
...
func (h *Handler) userUpdate(w http.ResponseWriter, r *http.Request) {
	...
	username := asString(req["user"])
	if username == "" {
		response.WriteJSON(w, response.ErrDefault("用户名不能为�?))
		return
	}
	...
	if err != nil {
		if err == sql.ErrNoRows {
			response.WriteJSON(w, response.ErrDefault("用户不存�?))
			return
		}
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	...
	if roleID == 0 {
		response.WriteJSON(w, response.ErrDefault("请不要作�?))
		return
	}
	...
	name := asString(req["name"]) // 解析前端传来的备注名�?
	pwd := asString(req["pwd"])
...
func (h *Handler) nodeCreate(w http.ResponseWriter, r *http.Request) {
	...
	// �?serverIp(serverIpV4/serverIpV6/intranetIp) 中选择第一个非空作�?serverIP
	// 优先使用 serverIp（兼容旧 API），否则�?serverIpV4/serverIpV6/intranetIp 中选择
...
func (h *Handler) nodeUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			response.WriteJSON(w, response.ErrDefault("节点不存�?))
			return
		}
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
...
func (h *Handler) nodeInstallDomestic(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			response.WriteJSON(w, response.ErrDefault("请先前往网站配置中设�?ip"))
			return
		}
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
...
func (h *Handler) nodeInstallOverseas(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.WriteJSON(w, response.Err(-2, fmt.Sprintf("获取最�?s失败�?v", releaseChannelLabel(channel), err)))
		return
	}
	...
	// 使用全局加速地址下载
	// install.sh �?main 分支下载（保证是最新的），二进制文件从 Releases 下载指定版本
...
func (h *Handler) tunnelGet(w http.ResponseWriter, r *http.Request) {
	...
	response.WriteJSON(w, response.ErrDefault("隧道不存�?))
}
...
func (h *Handler) tunnelDiagnose(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "不存�?) || strings.Contains(err.Error(), "不完�?) {
			response.WriteJSON(w, response.ErrDefault(err.Error()))
			return
		}
		response.WriteJSON(w, response.Err(-2, err.Error()))
...
func (h *Handler) userTunnelBatchUpdateStatus(w http.ResponseWriter, r *http.Request) {
	...
	if status != 1 && status != 2 {
		response.WriteJSON(w, response.ErrDefault("status 必须�?1（启用）�?2（禁用）"))
		return
	}
	...
	if len(ids) == 0 {
		response.WriteJSON(w, response.ErrDefault("有效�?ID 列表为空"))
		return
	}
...
func (h *Handler) forwardCreate(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.WriteJSON(w, response.Err(401, "无效的token或token已过�?))
		return
	}
	...
	if expiryTimeVal, ok := req["expiryTime"]; ok && expiryTimeVal != nil {
		...
		if expiryTime > 0 && expiryTime <= now {
			response.WriteJSON(w, response.ErrDefault("到期时间不能早于或等于当前时�?))
			return
		}
	}
	if roleID != 0 {
		if speedIDVal, ok := req["speedId"]; ok && speedIDVal != nil {
			response.WriteJSON(w, response.Err(-1, "普通用户无法设置限速规�?))
			return
		}
	}
	...
	if mode != "gost" && mode != "nftables" {
		response.WriteJSON(w, response.ErrDefault("ת��ģʽ��Ч����֧�� gost �� nftables"))
		return
	}
...
func (h *Handler) forwardUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, errForwardNotFound) {
			response.WriteJSON(w, response.ErrDefault("转发不存�?))
			return
		}
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	...
	if actorRole != 0 && hasSpeedID && requestedSpeedID != nil && !sameSpeedLimitSelection(forward.SpeedID, requestedSpeedID) {
		response.WriteJSON(w, response.Err(-1, "普通用户无法修改限速规�?))
		return
	}
	...
	if tunnelChanged && len(keptNodeIDs) > 0 {
		for _, nodeID := range keptNodeIDs {
			if delErr := h.deleteForwardServicesOnNodeBatch(forward, nodeID); delErr != nil {
				...
				warnings = append(warnings, fmt.Sprintf("节点 %s 清理旧转发监听失�? %v", nodeLabel, delErr))
			}
		}
	...
	if tunnelChanged && len(removedNodeIDs) > 0 {
		for _, nodeID := range removedNodeIDs {
			if delErr := h.deleteForwardServicesOnNodeBatch(forward, nodeID); delErr != nil {
				...
				warnings = append(warnings, fmt.Sprintf("节点 %s 清理旧隧道残留服务失�? %v", nodeLabel, delErr))
			}
		}
	}
	if len(warnings) > 0 {
...
func (h *Handler) forwardDiagnose(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "不存�?) || strings.Contains(err.Error(), "不能为空") || strings.Contains(err.Error(), "错误") {
			response.WriteJSON(w, response.ErrDefault(err.Error()))
			return
		}
		response.WriteJSON(w, response.Err(-2, err.Error()))
...
func (h *Handler) forwardBatchChangeTunnel(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.WriteJSON(w, response.ErrDefault("目标隧道不存�?))
		return
	}
	if targetTunnel.Status != 1 {
		response.WriteJSON(w, response.ErrDefault("目标隧道已禁�?))
		return
	}
	success := 0
	fail := 0
	...
	for _, id := range req.ForwardIDs {
		...
		if forward.TunnelID == req.TargetTunnelID {
			fail++
			failures = appendBatchFailureReason(failures, id, forward.Name, "规则已在目标隧道�?)
			continue
		}
		...
		if len(oldPorts) == 0 {
			fail++
			failures = appendBatchFailureReason(failures, id, forward.Name, "转发入口端口不存�?)
			continue
		}
		oldNodeIDs := forwardPortNodeIDs(oldPorts)
...
func (h *Handler) prepareTunnelCreateState(tx *gorm.DB, req map[string]interface{}, tunnelType int, excludeTunnelID int64) (*tunnelCreateState, error) {
	...
	for _, nodeID := range nodeIDs {
		...
		if err != nil {
			if strings.Contains(err.Error(), "不存�?) {
...
func (h *Handler) applyFederationRuntime(state *tunnelCreateState, localDomain string) ([]repo.FederationTunnelBinding, []federationRuntimeReleaseRef, error) {
	...
	for outIdx := range state.OutNodes {
		...
		if err != nil {
			h.releaseFederationRuntimeRefs(releaseRefs)
			return nil, nil, fmt.Errorf("远程节点 %s 运行时下发失�? %w", nodeDisplayName(node), err)
		}
	...
	for hopIdx := len(state.ChainHops) - 1; hopIdx >= 0; hopIdx-- {
		for nodeIdx := range state.ChainHops[hopIdx] {
			...
			for _, target := range nextTargets {
				...
				if targetNode == nil {
					h.releaseFederationRuntimeRefs(releaseRefs)
					return nil, nil, errors.New("节点不存�?)
				}
...
func (h *Handler) applyTunnelRuntime(state *tunnelCreateState) ([]int64, []int64, error) {
	...
	for _, inNode := range state.InNodes {
		...
		if _, err := h.sendNodeCommand(inNode.NodeID, "AddChains", chainData, true, false); err != nil {
			...
			return createdChains, createdServices, fmt.Errorf("入口节点 %s 下发转发链失�? %w", nodeDisplayName(state.Nodes[inNode.NodeID]), err)
		}
	...
	for i, hop := range state.ChainHops {
		...
		for _, chainNode := range hop {
			...
			if _, err := h.sendNodeCommand(chainNode.NodeID, "AddChains", chainData, true, false); err != nil {
				return createdChains, createdServices, fmt.Errorf("转发链节�?%s 下发转发链失�? %w", nodeDisplayName(state.Nodes[chainNode.NodeID]), err)
			}
			...
			if err := h.addTunnelServiceOnNode(chainNode.NodeID, state.TunnelID, serviceData); err != nil {
				return createdChains, createdServices, fmt.Errorf("转发链节�?%s 下发服务失败: %w", nodeDisplayName(state.Nodes[chainNode.NodeID]), err)
			}
...
func buildTunnelChainConfig(tunnelID int64, fromNodeID int64, targets []tunnelRuntimeNode, nodes map[int64]*nodeRecord, ipPreference string) (map[string]interface{}, error) {
	...
	if len(targets) == 0 {
		return nil, errors.New("转发链目标不能为�?)
	}
	...
	for idx, target := range targets {
		...
		protocol := defaultString(target.Protocol, "tls")
		// �?修复 1: 为所�?Relay Connector 启用 noDelay 模式（不限于 TLS�?
...
func buildTunnelChainServiceConfig(tunnelID int64, chainNode tunnelRuntimeNode, node *nodeRecord, nextHopCandidateCount int) []map[string]interface{} {
	...
	protocol := defaultString(chainNode.Protocol, "tls")
	// �?修复 1: 为所�?Relay Handler 启用 noDelay 模式（不限于 TLS�?
	// 确保连接错误立即暴露，故障转移机制正常工�?
...
func (h *Handler) upsertUserTunnel(req map[string]interface{}) error {
	...
	if lookupErr == sql.ErrNoRows {
		...
		if syncErr := h.syncUserTunnelForwards(userID, tunnelID); syncErr != nil {
			...
			if lookupErr != nil {
				return fmt.Errorf("下发失败且回滚失�? %v; 回滚查询错误: %w", syncErr, lookupErr)
			}

			if rollbackErr := h.repo.DeleteUserTunnel(insertedID); rollbackErr != nil {
				return fmt.Errorf("下发失败且回滚失�? %v; 回滚删除错误: %w", syncErr, rollbackErr)
			}
	...
	if syncErr := h.syncUserTunnelForwards(userID, tunnelID); syncErr != nil {
		...
		if rollbackErr != nil {
			return fmt.Errorf("下发失败且回滚失�? %v; 回滚错误: %w", syncErr, rollbackErr)
		}
