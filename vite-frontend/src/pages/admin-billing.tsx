...
import type { BalanceLogItem, RedeemCodeItem, DiscountCodeItem, ProductApiItem, UserApiItem } from "@/api/types";
...
function parseDateText(value: string): number | undefined {
  ...
  // Support YYYY-MM-DD or YYYY-MM-DD HH:mm format
...
export default function AdminBillingPage() {
  ...
  const loadAll = useCallback(async () => {
    if (!isFirstLoad.current) setRefreshingLogs(true);
    try {
      ...
      setLoading(false);
      if (!isFirstLoad.current) setRefreshingLogs(false);
      isFirstLoad.current = false;
    }
  }, [logPage]);

  // User filter change: silent refresh without full-screen loading
  useEffect(() => {
  ...
  return (
    <AnimatedPage className="px-3 lg:px-6 py-8">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold">余额与营销</h1>
      </div>
            ...
            <CardBody className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                <div className="sm:col-span-3 lg:col-span-3 space-y-2">
                  ...
                  <div className="flex gap-2">
                    <input className="flex-1 h-10 px-3 py-2 border border-default-200 rounded-lg bg-default-50 text-sm" value={redeemCodeVal}
                ...
                {redeemType === "plan" ? (
                  ...
                  <div className="space-y-2">
                    <label className="text-sm text-gray-400">余额金额</label>
                    <input type="number" step="0.01" min="0.01" className="w-full h-10 px-3 py-2 border border-default-200 rounded-lg bg-default-50 text-sm"
                      value={redeemAmount} onChange={(e) => setRedeemAmount(e.target.value)} />
                  </div>
                )}
                <div className="space-y-2">
                  <label className="text-sm text-gray-400">数量</label>
                  <input type="number" min="1" max="500" className="w-full h-10 px-3 py-2 border border-default-200 rounded-lg bg-default-50 text-sm"
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <input className="flex-1 h-10 px-3 py-2 border border-default-200 rounded-lg bg-default-50 text-sm"
                    ...
                    <Dropdown>
                      ...
                      <DropdownMenu>
                        <DropdownItem onPress={() => {
                          ...
                          setRedeemStartsAt(`${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, '0')}${String(d.getDate()).padStart(2, '0')}`);
                        }}>1 月后</DropdownItem>
                        <DropdownItem onPress={() => {
                          const d = new Date();
                    ...
                    <Dropdown>
                      ...
                      <DropdownMenu>
                        <DropdownItem onPress={() => {
                          const d = new Date();
                          d.setMonth(d.getMonth() + 1);
                          setRedeemExpiresAt(`${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, '0')}${String(d.getDate()).padStart(2, '0')}`);
                        }}>1 月后</DropdownItem>
                        <DropdownItem onPress={() => {
                          const d = new Date();
                          d.setMonth(d.getMonth() + 3);
          ...
          <Card>
            ...
            <CardBody className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="space-y-2">
                  <label className="text-sm text-gray-400">折扣码</label>
                  <div className="flex gap-2">
                    <input className="flex-1 h-10 px-3 py-2 border border-default-200 rounded-lg bg-default-50 text-sm" value={dcCode}
                ...
                <div className="space-y-2">
                  <label className="text-sm text-gray-400">{dcType === "percent" ? "百分比" : "金额"}</label>
                  <input type="number" min="0" step={dcType === "percent" ? "1" : "0.01"} className="w-full h-10 px-3 py-2 border border-default-200 rounded-lg bg-default-50 text-sm"
                    value={dcValue} onChange={(e) => setDcValue(e.target.value)} />
                </div>
                <div className="space-y-2">
                  <label className="text-sm text-gray-400">可用次数</label>
                  <input type="number" min="0" className="w-full h-10 px-3 py-2 border border-default-200 rounded-lg bg-default-50 text-sm"
                    ...
                    <Dropdown>
                      ...
                      <DropdownMenu>
                        <DropdownItem onPress={() => {
                          const d = new Date();
                          d.setMonth(d.getMonth() + 1);
                          setDcStartsAt(`${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, '0')}${String(d.getDate()).padStart(2, '0')}`);
                        }}>1 月后</DropdownItem>
                        <DropdownItem onPress={() => {
                          const d = new Date();
                          d.setMonth(d.getMonth() + 3);
                    ...
                    <Dropdown>
                      ...
                      <DropdownMenu>
                        <DropdownItem onPress={() => {
                          const d = new Date();
                          d.setMonth(d.getMonth() + 1);
                          setDcExpiresAt(`${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, '0')}${String(d.getDate()).padStart(2, '0')}`);
                        }}>1 月后</DropdownItem>
                        <DropdownItem onPress={() => {
                          const d = new Date();
                          d.setMonth(d.getMonth() + 3);
